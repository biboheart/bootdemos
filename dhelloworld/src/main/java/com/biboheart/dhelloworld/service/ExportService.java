package com.biboheart.dhelloworld.service;

import com.biboheart.brick.exception.BhException;
import com.biboheart.brick.utils.CheckUtils;
import com.biboheart.brick.utils.ListUtils;
import com.biboheart.brick.utils.TimeUtils;
import com.biboheart.dhelloworld.entity.*;
import com.biboheart.dhelloworld.entity.Dictionary;
import com.biboheart.dhelloworld.repository.*;
import com.biboheart.dhelloworld.service.model.CostInfo;
import com.biboheart.dhelloworld.service.model.DiagnoseInfo;
...
public class ExportService {
    ...
    public void exportDBF(String jzids, HttpServletResponse response) throws Exception {
        ...
        String fileName = "N041_金华田氏医院.DBF";
...
//        List<HomeInfo> homeInfoList = hisDock.listHomeInfo(jzids, null, null, null);
        ...
        for (Home home : homeInfoList) {
            ...
            n041.setNL(home.getAge() / 100000); // 年龄
            ...
            if (null != diagnoseInfo) {
                n041.setBLZD(diagnoseInfo.getIcd()); // 病理诊断
                n041.setJBMM(diagnoseInfo.getDiagnosis()); // 病理诊断编码
            } else {
            ...
            if (null != diagnoseInfo) {
                n041.setWBYY(diagnoseInfo.getIcd()); // 病理诊断
                n041.setH23(diagnoseInfo.getDiagnosis()); // 病理诊断编码
            } else {
            ...
            if (null != diagnoseInfo) {
                n041.setMZZD(diagnoseInfo.getIcd()); // 病理诊断
                n041.setJBBM(diagnoseInfo.getDiagnosis()); // 病理诊断编码
            } else {
            ...
            List<DiagnoseInfo> otherDiagnose = new ArrayList<>();
            ...
            for (DiagnosisInfo numberDiagnosis : numberDiagnosisList) {
                ...
                if(Integer.valueOf(1).equals(numberDiagnosis.getTag())) {
                    n041.setZYZD(numberDiagnosis.getIcd()); // 主要诊断 his出院诊断
                    n041.setJBDM(numberDiagnosis.getDiagnosis());
                    ...
                    /*String zdlx;
                    switch (numberDiagnosis.getType()) {
                        case 5:
                            zdlx = "医院感染";
                            break;
                        case 7:
                    ...
                    String QTZD = "QTZD" + (i + 1);
                    String JBDM = "JBDM" + (i + 1);
                    String RYBQ = "RYBQ" + (i + 1);
                    ...
                    setQTZD.invoke(n041, numberDiagnosis.getIcd());
                    setJBDM.invoke(n041, numberDiagnosis.getDiagnosis());
            ...
            if (null != charge) {
                n041.setZFY(BigDecimal.valueOf(charge.getMedfeeSumamt()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue());
                n041.setZFJE(BigDecimal.valueOf(charge.getSelfFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue());
                n041.setYLFUF(BigDecimal.valueOf(charge.getOrdnMedServfee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 综合医疗服务类：(1)一般医疗服务费
                n041.setZLCZF(BigDecimal.valueOf(charge.getOrdnTrtOprtFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 一般治疗操作费
                n041.setHLF(BigDecimal.valueOf(charge.getNursFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 护理费住院费
                n041.setQTFY(BigDecimal.valueOf(charge.getComMedServOthFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 其他费用
                n041.setBLZDF(BigDecimal.valueOf(charge.getPalgDiagFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 诊断类：(5)病理诊断费
                n041.setSYSZDF(BigDecimal.valueOf(charge.getLabDiagFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 实验室诊断费
                n041.setYXXZDF(BigDecimal.valueOf(charge.getRdhyDiagFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 影像学诊断费
                n041.setLCZDXMF(BigDecimal.valueOf(charge.getClncDiseFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 临床诊断项目费
                n041.setFSSZLXMF(BigDecimal.valueOf(charge.getNsrgtrtItemFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 治疗类：(9)非手术治疗项目费
                n041.setWLZLF(BigDecimal.valueOf(charge.getClncPhysTrtFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 临床物理治疗费
                n041.setSSZLF(BigDecimal.valueOf(charge.getRgtrtTrtFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // (10)手术治疗费
                n041.setMAF(BigDecimal.valueOf(charge.getAnstFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 麻醉费
                n041.setSSF(BigDecimal.valueOf(charge.getOprnFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 手术费
                n041.setKFF(BigDecimal.valueOf(charge.getRhabFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 康复类：(11)康复费
                n041.setZYZLF(BigDecimal.valueOf(charge.getTcmTrtFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 中医类:(12)中医治疗费
                n041.setXYF(BigDecimal.valueOf(charge.getWmfee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 西药类:(13)西药费
                n041.setKJYWF(BigDecimal.valueOf(charge.getAbtlMednFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 抗菌药物费
                n041.setZCYF(BigDecimal.valueOf(charge.getTcmpatFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 中药类:(14)中成药费
                n041.setZCYF1(BigDecimal.valueOf(charge.getTcmherbFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 中草药费
                n041.setXF(BigDecimal.valueOf(charge.getBloFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 血液和血液制品类:(16)血费
                n041.setBDBLZPF(BigDecimal.valueOf(charge.getAlbuFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 白蛋白类制品费
                n041.setQDBLZPF(BigDecimal.valueOf(charge.getGlonFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 球蛋白类制品费
                n041.setNXYZLZPF(BigDecimal.valueOf(charge.getClotfacFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 凝血因子类制品费
                n041.setXBYZLZPF(BigDecimal.valueOf(charge.getCykiFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 细胞因子类制品费
                n041.setHCYYCLF(BigDecimal.valueOf(charge.getExamDspoMatlFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 耗材类:(21)检查用一次性医用材料费
                n041.setYYCLF(BigDecimal.valueOf(charge.getTrtDspoMatlFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 治疗用一次性医用材料费
                n041.setYCXYYCLF(BigDecimal.valueOf(charge.getOprnDspoMatlFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 手术用一次性医用材料费
                n041.setQTF(BigDecimal.valueOf(charge.getOthFee()).divide(BigDecimal.valueOf(10000), 2, RoundingMode.HALF_UP).doubleValue()); // 其他费
            }
            n041Vos.add(n041);
        }
        return n041Vos;
