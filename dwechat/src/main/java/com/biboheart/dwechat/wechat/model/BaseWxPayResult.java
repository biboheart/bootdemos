...
import com.biboheart.dwechat.wechat.utils.SignUtils;
import com.biboheart.dwechat.wechat.utils.XStreamInitializer;
...
public abstract class BaseWxPayResult implements Serializable {
    ...
    String getXmlValue(String... path) {
        Document doc = this.getXmlDoc();
        String expression = String.format("/%s//text()", String.join("/", path));
        try {
     ...
     */
    public void checkResult(String mchKey, String signType, boolean checkSuccess) throws WxPayException {
        //校验返回结果签名
        Map<String, String> map = toMap();
        if (getSign() != null && !SignUtils.checkSign(map, signType, mchKey)) {
